/** Working schedules (A3). Owner: Track B. */
...
schedules.get('/:id', can('schedules', 'read'), ah(async (req, res) => {
  ...
  res.json({ data: { ...s, lines, hours_per_week: Math.round(weeklyHours(lines) * 100) / 100 } });
}));
...
schedules.post('/', can('schedules', 'write'), ah(async (req, res) => {
  ...
  const id = await tx(async (c) => {
    const { rows } = await c.query(
      'INSERT INTO working_schedules (name,type) VALUES ($1,$2) RETURNING id', [name, type]
    );
  ...
  for (const l of lines) {
    await client.query(
      `INSERT INTO schedule_lines (schedule_id,day_of_week,start_time,end_time,break_minutes)
       VALUES ($1,$2,$3,$4,$5)`,
      [scheduleId, l.day_of_week, l.start_time, l.end_time, l.break_minutes || 0]
    );
  }
}
